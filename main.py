...
from Database import get_cpu_from_stats, get_ram_from_stats, get_disk_from_stats, get_network_from_stats, save_entry
...
async def disk(user_id: int, pc_id: tuple[int] = (), period_start: str = dt(1999, 1, 1).isoformat(),
               period_end: str = dt(2030, 1, 1).isoformat()):
    return get_disk_from_stats(user_id, pc_id, dt.fromisoformat(period_start), dt.fromisoformat(period_end))
